...
def load_all_gcpt(
    ...
    for benchspec in data:
        ...
        for point in data_iterator:
            ...
            expected_checkpoints_num += 1
            hour = -1
            # FIXME: In order not to change the run json and get the time, here it's global time json temporarily
            # the global time json is generated by `gcpt_run_time_json.py`
            if (
                not no_ref_run_time
                and time_data.get(benchspec)
                and time_data[benchspec].get("times")
                and time_data[benchspec]["times"].get(point)
            ):
            ...
            if state_match and report:
                if gcc12Enable:
                    ...
                    if len(error_json[benchspec]) == 0:
                        del error_json[benchspec]
        expected_minimal_coverage = min(expected_minimal_coverage, float(weight_sum))
    ...
    if not report:
        if sorted_by is not None:
            ...
            all_gcpt = sorted(
                all_gcpt, key=lambda x: -1 if x.benchspec == "mcf" else -float(x.weight)
            )
...
def xs_run(
    ...
    try:
        ...
        for index in tqdm(range(max_num)):
            ...
            run_cmd = (
                base_arguments + [workload.get_bin_path()] + ["-s", f"{random_seed}"]
            )
            if simFrontend:
            ...
            while not assigned:
                ...
                for sidx in range(server_index_flag, server_index_max):
                    if dry_run and verbose:
                        ...
                        print(run_cmd)
                    if servers[sidx].assign(
                        f"{workload}",
                        run_cmd,
                        threads,
                        xs_path,
                        workload.get_out_path(),
                        workload.get_err_path(),
                        dry_run,
                        verbose,
                    ):
                        assigned = True
...
def get_all_manip():
    ...
    l3cache_mpki_load = perf.PerfManip(
        ...
        func=lambda fire1, fire2, fire3, fire4, fire5, fire6, fire7, fire8, instr: 1000
...
def xs_report(
    ...
    for benchspec in gcpt_ipc:
        ...
        total_cpi = (
            sum(map(lambda info: info[0] / info[1], gcpt_ipc[benchspec])) / total_weight
        )
        if gcc12Enable:
        ...
        spec_weight_list[spec_name] = spec_weight_list.get(spec_name, []) + [
            total_weight
        ]
        spec_instr_list[spec_name] = spec_instr_list.get(spec_name, []) + [num_instr]
    for key in spec_weight_list:
        spec_weight[key] = np.sum(
            np.multiply(spec_weight_list[key], spec_instr_list[key])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "gcpt_path", metavar="gcpt_path", type=str, help="path to gcpt checkpoints"
    )
    parser.add_argument(
        "json_path", metavar="json_path", type=str, help="path to gcpt json"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="path to xs emulator elf (overrides default xs_path/build/emu)",
    )
    ...
    parser.add_argument(
        "--threads", "-T", default=1, type=int, help="number of emu threads"
    )
    parser.add_argument(
        "--server-list",
        "-L",
        type=str,
        help='server list, like "172.28.9.104 172.28.9.107", support alias',
    )
    parser.add_argument(
        "--report", "-R", action="store_true", default=False, help="report only"
    )
    parser.add_argument(
        "--show",
        "-S",
        action="store_true",
        default=False,
        help="show list of gcpt only",
    )
    parser.add_argument(
        "--debug", "-D", action="store_true", default=False, help="debug options"
    )
    parser.add_argument(
        "--check", "-C", action="store_true", default=False, help="debug options"
    )
    parser.add_argument(
        "--sim-frontend",
        "-F",
        action="store_true",
        default=False,
        help="use trace for sim frontend",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--jobs", "-j", default=32, type=int, help="processing files in 'j' threads"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        default=False,
        help="continue to exe, ignore the aborted and success tests",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=False,
        help="does not run real simulation",
    )
    parser.add_argument(
        "--verbose",
        "-v",
        action="store_true",
        default=False,
        help="display more outputs",
    )
    parser.add_argument(
        "--dump-db",
        action="store_true",
        default=False,
        help="dump database for necessary tests",
    )
    parser.add_argument(
        "--benchmarks",
        default="",
        type=str,
        help="Specific benchmarks to run (comma-separated), leave empty to run all",
    )
